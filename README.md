...
- `Java`高级（`hard`）：包括`JVM`、类加载、字节码工程、`JUC`、一些源代码等。

另外除了这些之外，还会收录制作`JavaEE`技术相关的内容，这些内容可能已经不适合时代而被淘汰，但是为了更好地解释现代的技术，还是会简单了解一下。

喜欢的给个⭐吧o(*￣▽￣*)ブ。

## 😘阅读指南

所有的技术都会以分支的形式进行命名，格式为`序号-java-层次-技术点`，如：`1-java-advanced-reflect`代表是`Java`进阶反射技术，可以打开相对应的分支进行学习。

一般一个分支会包括一个`README.md`和一个或者多个`DEMO`，`README.md`用于辅助`Demo`和知识点的讲解，`Demo`为`maven`项目。

## 😘Demo运行

项目一个知识点一个分支的方式进行管理，在编写的时候采用`idea`多模块的形式进行，如果单单下载某个分支的`Demo`，可能需要修改一下`pom`文件，具体你可以在这里找到各个`pom`文件，替换掉从分支下载的`Demo`内自带的`pom`文件即可运行
